...
public sealed class OneDriveVolumeSource : IVolumeSource
{
    ...
    internal OneDriveVolumeSource(IOneDriveClient apiClient, bool allowWriteAccess = false)
    {
        _apiClient = apiClient ?? throw new ArgumentNullException(nameof(apiClient));
        _allowWriteAccess = allowWriteAccess;
    }

    public string Id => "onedrive::root";
    ...
    public string Root => OneDrivePath.RootPath;
    ...
    public IFileEntry GetEntry(string path)
    {
        ...
        return new OneDriveFileEntry(
            FullPath: OneDrivePath.BuildPath(normalizedRelativePath),
    ...
    public IEnumerable<IFileEntry> Enumerate(string path)
    {
        ...
        return items
            .Select(item => (IFileEntry)new OneDriveFileEntry(
                FullPath: OneDrivePath.BuildPath(CombineRelativePath(normalizedRelativePath, item.Name)),
                Name: item.Name,
                IsDirectory: item.IsDirectory,
                Size: item.Size,
                LastWriteTimeUtc: item.LastWriteTimeUtc,
